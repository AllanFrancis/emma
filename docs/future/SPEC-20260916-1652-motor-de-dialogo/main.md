...
- [ ] Nenhuma chave de API no bundle do cliente, verificado por inspeção do build
- [ ] Prompt gerado por função pura a partir do `Intent`, com teste que não faz rede
- [ ] Chamada usa `json_schema` com `strict: true` e o `turn-schema.json` como fonte única
- [ ] Resposta é validada contra o schema e contra a evidência citada no servidor, antes de chegar ao cliente
- [ ] `json_validate_failed` recebe retry e, persistindo, cai no roteiro da missão — turno nunca morre na tela, com teste do caminho de falha
- [ ] O prompt declara que a entrada é fala transcrita (DEC-20260916-0312)
- [ ] Rate limit por tokens vira mensagem compreensível ao aluno, não erro cru
- [ ] Trocar o adapter de provedor não altera nenhum arquivo do núcleo pedagógico
- [ ] Zero decisão pedagógica no motor: nenhuma política de nível, suporte ou correção neste módulo
