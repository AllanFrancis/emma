...
const SNAP_PLACEHOLDER =
  /início — nada feito ainda|<primeiro passo concreto>|<primeiro passo>|<fase>|<última decisão>/;
...
const SECRET_PATTERNS = [
  ...
  [
    "connection-string",
    /\b[a-zA-Z][a-zA-Z0-9+.-]{1,24}:\/\/[^\s:@/]{1,64}:[^\s@/]{1,128}@[^\s]{1,256}/,
  ],
...
function die(msg) {
  process.stderr.write(`[specctl] ERRO: ${msg}\n`);
  process.exit(1);
}
function out(msg) {
...
function walkFiles(
  dir,
  { skipDirs = [".git", "node_modules", "tmp"], exts = null } = {},
  acc = [],
) {
...
function git(args, cwd) {
  ...
  if (!r.ok)
    die(
      `git ${args.join(" ")} falhou: ${r.err || "(sem stderr)"} — corrija o estado do git antes de continuar`,
    );
  return r.out;
}
...
function gitTry(args, cwd) {
  ...
  if (!r.ok) {
    process.stderr.write(
      `[specctl] AVISO: git ${args.join(" ")} falhou: ${r.err || "(sem stderr)"}\n`,
    );
    return null;
  }
  return r.out;
...
function protectedBranches(root) {
  try {
    ...
    if (Array.isArray(m.protected_branches) && m.protected_branches.length)
      return m.protected_branches;
  } catch {
    /* sem manifesto: default */
  }
  return DEFAULT_PROTECTED;
}
function isProtectedBranch(b, root) {
  return !!b && protectedBranches(root).includes(b);
}
...
function findRoot(start) {
  ...
  for (;;) {
    if (exists(P(dir, "docs", ".spec-system.json")) || exists(P(dir, "docs", "RULES.md")))
      return dir;
...
function requireRoot() {
  ...
  if (!r)
    die(
      "projeto não encontrado — nenhum docs/.spec-system.json (nem docs/RULES.md) subindo a partir de " +
        process.cwd(),
    );
  return r;
}
function manifest(root) {
  ...
  try {
    return { ...def, ...JSON.parse(read(p)) };
  } catch (e) {
    process.stderr.write(`[specctl] AVISO: manifesto inválido (${e.message}) — usando defaults\n`);
    return def;
  }
}
    ...
    for (const e of fs.readdirSync(base, { withFileTypes: true })) {
      if (e.isDirectory())
        specs.push({
          ...
          phase,
          dir: P(base, e.name),
          v3: exists(P(base, e.name, "state.md.v3.bak")),
        });
    }
  }
  return specs;
...
function activeSpecs(root) {
  return listSpecs(root).filter((s) => s.phase === "active");
}
  ...
  for (let i = 0; i < pm.sections.length; i++) {
    ...
    if (s.level === 2 && pred(s.title)) {
      ...
      for (let j = i + 1; j < pm.sections.length; j++) {
        if (pm.sections[j].level <= 2) {
          end = pm.sections[j].line;
          break;
        }
      }
      return [s.line, end];
...
function getCriteria(pm) {
  ...
  for (let i = r[0] + 1; i < r[1]; i++) {
    ...
    if (m)
      outArr.push({ line: i, checked: m[1].toLowerCase() === "x", text: m[2], raw: pm.lines[i] });
  }
  return outArr;
}
...
function claimPath(root, id) {
  return P(root, "docs", "claims", `${id}.md`);
}
function removeClaim(root, id) {
  const p = claimPath(root, id);
...
function moveSpec(spec, root, toPhase) {
  ...
  try {
    fs.renameSync(spec.dir, dst);
  } catch {
    fs.cpSync(spec.dir, dst, { recursive: true });
...
function cmdInit(flags) {
  ...
  if (exists(manPath) && !flags.force)
    die(`já instalado (${relOf(root, manPath)}) — use --force para regravar o manifesto`);
  ...
  for (const line of ["docs/active/**/tmp/", ".scratch/"]) {
    if (!giLines.includes(line))
      giOut = (giOut && !giOut.endsWith("\n") ? giOut + "\n" : giOut) + line + "\n";
  }
...
`,
  ...
  if (!exists(roadmapPath))
    writeFile(
      roadmapPath,
      `# ROADMAP — ordem de execução do backlog avulso
...
function cmdNew(pos, flags) {
  ...
  if (!slug)
    die(
      "uso: specctl new <slug> [--future] [--porte P|M|G] [--owner @x] [--features a,b] [--program p] [--workspace w]",
    );
  if (!/^[a-z0-9][a-z0-9-]{0,48}$/.test(slug))
    die(`slug inválido '${slug}' — use kebab-case [a-z0-9-]`);
  ...
  if (!future && isProtectedBranch(branch, root)) {
    die(
      `R.2: SPEC ativa é proibida na branch protegida '${branch}' (${protectedBranches(root).join("/")}) — trabalho vive sempre em branch própria. Caminho correto: git checkout -b feature/${slug} e rode o comando lá — ou --future para rascunho em docs/future/.`,
    );
  }
  ...
  if (!future) {
    ...
    appendLog(
      dir,
      "ativação",
      `SPEC criada e ativada (${owner}, branch ${branch || "—"}, base ${base})`,
    );
    writeFile(
      claimPath(root, id),
      makeClaim({
  ...
  out(
    `criada ${relOf(root, dir)} (${future ? "rascunho em future/ — status draft" : "ativa — claim registrado"})`,
  );
...
function cmdActivate(pos, flags) {
  ...
  if (spec.phase !== "future")
    die(
      `activate: ${spec.id} está em ${spec.phase}/ — só rascunhos em future/ podem ser ativados (pausadas: use resume)`,
    );
  const pm = parseMain(read(P(spec.dir, "main.md")));
  if (pm.fields.Status === "paused")
    die(`${spec.id} está pausada — caminho correto: node scripts/specctl.mjs resume ${spec.id}`);
  const branch = flags.branch || currentBranch(root);
  if (isProtectedBranch(branch, root))
    die(
      `R.2: não ative SPEC na branch protegida '${branch}' (${protectedBranches(root).join("/")}). Caminho correto: git checkout -b feature/<slug> e rode activate lá.`,
    );
  ...
  if (manifest(root).team === "multi") {
    ...
    out(
      `  abra o PR para ${base} (auto-merge se permitido) e volte: git checkout ${branch || "<branch>"}`,
    );
  }
  out(JIT_FACTS); // C2 — bloco estável (≤900B, sem NOW): permitido SÓ em new/activate/close--dry
}
...
function cmdPause(pos, flags) {
  ...
  if (spec.phase !== "active")
    die(`pause: ${spec.id} está em ${spec.phase}/ — só SPECs ativas podem ser pausadas`);
  ...
  out(
    `pausada: docs/future/${spec.id} (claim removido). Retome com: node scripts/specctl.mjs resume ${spec.id}`,
  );
  void dst;
}
...
function cmdResume(pos) {
  ...
  const spec = findSpec(root, pos[0]);
  if (spec.phase !== "future")
    die(`resume: ${spec.id} está em ${spec.phase}/ — só pausadas em future/ podem ser retomadas`);
  const pm = parseMain(read(P(spec.dir, "main.md")));
  if (pm.fields.Status !== "paused")
    die(
      `${spec.id} tem status '${pm.fields.Status}' — rascunhos usam: node scripts/specctl.mjs activate ${spec.id}`,
    );
  const branch = currentBranch(root);
  if (isProtectedBranch(branch, root))
    die(
      `R.2: não retome SPEC na branch protegida '${branch}' (${protectedBranches(root).join("/")}). Caminho correto: git checkout -b feature/<slug> e rode resume lá.`,
    );
  let txt = read(P(spec.dir, "main.md"));
  txt = setField(txt, "Status", "active");
...
function cmdReopen(pos, flags) {
  const root = requireRoot();
  const spec = findSpec(root, pos[0]);
  if (spec.phase !== "archive")
    die(
      `reopen: ${spec.id} está em ${spec.phase}/ — só SPECs arquivadas reabrem (pausada: resume; descartada não reabre, crie SPEC nova citando-a)`,
    );
  const motivo = flags.motivo;
  if (!motivo)
    die(
      'reopen exige --motivo "..." (por que o fechamento foi indevido — ex.: critério aceito sem autorização R.6.2)',
    );
  const branch = currentBranch(root);
  if (isProtectedBranch(branch, root))
    die(
      `R.2: não reabra SPEC na branch protegida '${branch}' (${protectedBranches(root).join("/")}). Caminho correto: git checkout -b feature/<slug> e rode reopen lá.`,
    );
  let txt = read(P(spec.dir, "main.md"));
  txt = setField(txt, "Status", "active");
  if (/^\*\*Reaberta em:\*\*/m.test(txt))
    txt = setField(txt, "Reaberta em", `${now()} — ${motivo}`);
...
function featureConcludedOk(root, feat, id) {
  ...
  if (!scope.includes(pref))
    return {
      ok: false,
      why: `R.7: adicione ${pref} em '### Concluídas' de docs/features/${feat}.md (validação por CONTEÚDO)`,
    };
  return { ok: true };
}
...
function cmdArchive(pos) {
  ...
  if (spec.phase !== "active")
    die(
      `archive: ${spec.id} está em ${spec.phase}/ — só SPECs ativas são arquivadas (rascunho abandonado: discard)`,
    );
  ...
  if (!crit || !crit.length)
    pend.push('seção "## Critério de aceite" sem checkboxes — contrato R.6.2 vazio');
  else
    for (const c of crit) {
      if (!c.checked && !ACEITO_RE.test(c.raw)) {
        pend.push(
          `critério aberto sem marcador [aceito-incompleto: "citação literal do usuário" YYYY-MM-DD HH:MM]: "${truncate(c.text, 70)}" (R.6.2 — só o USUÁRIO aceita incompleto)`,
        );
      }
      if (c.checked && !hasTs(c.raw))
        pend.push(`checkbox [x] sem timestamp: "${truncate(c.text, 70)}" (R.6)`);
    }
  const dgp = P(spec.dir, "digest.md");
  if (!exists(dgp))
    pend.push(
      "digest.md ausente — obrigatório no fechamento (≤2.000 bytes; ver docs/rules/lifecycle.md)",
    );
  else {
    const db = digestMeasure(read(dgp));
    if (db > 2000)
      pend.push(
        `digest.md com ${db}B > 2000B (R.16) — re-gere dentro do cap: node scripts/specctl.mjs digest ${spec.id} --fix`,
      );
  }
  ...
  if (!exists(jp)) pend.push("journal.md ausente");
    ...
    const jtxt = read(jp);
    if (!/^## \d{4}-\d{2}-\d{2} \d{2}:\d{2} — (?:\[MARCO\] )?\[conclusão\]/m.test(jtxt))
      pend.push("entrada [conclusão] ausente no LOG do journal");
    ...
    if (SNAP_PLACEHOLDER.test(snap))
      pend.push(
        'SNAPSHOT ainda com placeholders do template ("Onde tô/Próximo passo/Fase" não preenchidos) — sobrescreva o SNAPSHOT do journal refletindo a conclusão antes de fechar',
      );
  }
  if ((pm.fields["Commit final"] || "—").trim() === "—")
    pend.push("**Commit final:** não preenchido (R.6.1)");
...
function cmdDiscard(pos, flags) {
  const root = requireRoot();
  const spec = findSpec(root, pos[0]);
  if (spec.phase !== "active" && spec.phase !== "future")
    die(`discard: ${spec.id} está em ${spec.phase}/ — nada a descartar`);
  const motivo = flags.motivo;
  if (!motivo)
    die('discard exige --motivo "..." (R.5: descarte preserva aprendizado com justificativa)');
  const mainPath = P(spec.dir, "main.md");
  let txt = read(mainPath);
  ...
  out(
    `lições/gotchas: refine o digest se preciso — node scripts/specctl.mjs digest ${spec.id} --stdin`,
  );
}
...
function genArchiveIndex(root) {
  ...
  for (const spec of listSpecs(root)
    .filter((x) => x.phase === "archive" || x.phase === "discard")
    .sort((a, b) => (a.id < b.id ? -1 : 1))) {
    const pm = parseMain(readSafe(P(spec.dir, "main.md")));
    const status = pm.fields.Status || (spec.phase === "archive" ? "done" : "discarded");
    rows.push(
      `${spec.id} | ${cleanCell(status)} | ${cleanCell(pm.fields.Features)} | ${cleanCell(pm.fields.Keywords)} | ${cleanCell(pm.fields.Resumo)}`,
    );
  }
  let s = `# ARCHIVE-INDEX\n\n${GERADO_MARK}\n\n`;
...
function genDeferred(root) {
  ...
  for (const spec of listSpecs(root).sort((a, b) => (a.id < b.id ? -1 : 1))) {
    ...
    for (const c of getCriteria(pm) || []) {
      ...
      if (!m) continue;
      const critTxt = c.text
        .replace(CITE_RE, "")
        .replace(/\s*\|\s*verify:.*$/, "")
        .trim();
      rows.push(
        `- ${spec.id} (${spec.phase}) | ${cleanCell(truncate(critTxt, 90))} | "${truncate(m[1], 70)}" ${m[2]}`,
      );
    }
  }
...
function cmdIndex(opts = {}) {
  ...
  if (!opts.quiet)
    out(
      "regenerados: docs/INDEX.md, docs/ARCHIVE-INDEX.md, docs/PROGRAMS.md e docs/DEFERRED.md (determinístico, sem timestamp)",
    );
}
...
function computeProgramState(prog, statusOf) {
  ...
  for (const n of prog.nodes) {
    ...
    if (st === "paused") {
      paused.push(n.id);
      continue;
    } // visível, mas não pegável — retomar é decisão (resume)
    if (st === "draft") {
...
function genProgramsIndex(root) {
  ...
  for (const { prog, state } of entries) {
    ...
    if (state.paused.length)
      block += `- pausados: ${state.paused.join(", ")} (retomar: specctl resume)\n`;
...
function extractRfs(txt) {
  ...
  const s = new Set();
  for (const m of clean.matchAll(/^\s*(?:#{1,6}\s+|[-*]\s+|\d+\.\s+)?\*{0,2}RF-(\d+)/gm))
    s.add(+m[1]);
...
function lintJournal(root, jp, err, warn2, v3) {
  const rep = (r, m) => {
    if (v3 && warn2) warn2(r, m, { stage1: true });
    else err(r, m);
  };
  ...
  const snapIdx = [];
  lines.forEach((l, i) => {
    if (/^## SNAPSHOT\b/.test(l)) snapIdx.push(i);
  });
  if (!snapIdx.length) err(relp, 'journal sem heading "## SNAPSHOT" (formato §3.3)');
  else {
    if (snapIdx[0] >= 60)
      err(relp, `SNAPSHOT começa na linha ${snapIdx[0] + 1} — DEVE caber nas primeiras 60 linhas`);
    if (snapIdx.length > 1)
      err(relp, 'heading "## SNAPSHOT" duplicado — SNAPSHOT é único e sobrescrito');
  }
  const logIdx = lines.findIndex((l) => /^## LOG\b/.test(l));
  if (logIdx >= 0 && snapIdx.length && logIdx > 60)
    err(
      relp,
      `SNAPSHOT termina na linha ${logIdx + 1} — DEVE caber nas primeiras 60 linhas (o brief trunca em 60)`,
    );
  const snapStart = snapIdx.length ? snapIdx[0] : -1;
  const snapEnd = logIdx >= 0 ? logIdx : lines.length;
  lines.forEach((l, i) => {
    if (
      (/^#{1,6}\s.*TL;?DR/i.test(l) || /^\*\*TL;?DR/i.test(l)) &&
      (i < snapStart || i >= snapEnd)
    ) {
      err(
        relp,
        `TL;DR fora do SNAPSHOT (linha ${i + 1}) — o único sumário sobrescrito é o SNAPSHOT`,
      );
    }
  });
  if (logIdx >= 0) {
    const entryRe = new RegExp(
      `^## \\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2} — \\[(MARCO|${LOG_TYPES.join("|")})\\]`,
    );
    for (let i = logIdx + 1; i < lines.length; i++) {
      if (/^##\s/.test(lines[i]) && !entryRe.test(lines[i])) {
        rep(
          relp,
          `entrada de LOG inválida (linha ${i + 1}): "${truncate(lines[i], 60)}" — formato: ## YYYY-MM-DD HH:MM — [tipo] Título; tipos: ${LOG_TYPES.join(" ")}`,
        );
      }
    }
  }
...
function runLint(root, opts = {}) {
  ...
  if (!exists(manPath))
    warn(
      ...
      "manifesto ausente — modo compatibilidade v3 (rode a skill spec-system-init, modo UPGRADE)",
    );
  else {
    try {
      JSON.parse(read(manPath));
    } catch (e) {
      err("docs/.spec-system.json", `JSON inválido: ${e.message}`);
    }
  }

  // estrutura de pastas
  for (const d of ["features", "active", "future", "archive", "discard", "claims", "programs"]) {
    if (!isDir(P(root, "docs", d)))
      err(`docs/${d}/`, "pasta obrigatória ausente (estrutura v4 — rode specctl init)");
  }
  ...
  const feats = featureFiles(root);
  if (!tax && feats.length)
    warn("docs/TAXONOMY.md", "ausente — R.13 exige vocabulário canônico de áreas");
  ...
  for (const f of feats) {
    ...
    if (/###\s+Em execução/.test(txt))
      warn(
        relp,
        'seção "Em execução" foi removida na v4 — o trabalho em voo vive em docs/claims/ (specctl activate)',
      );
    for (const m of stripFences(txt).matchAll(
      /^\s*-\s*(?:\*\*)?(DEC-\d{8}-\d{4}(?:-[a-z0-9-]+)?)/gm,
    )) {
      // DEC de EXEMPLO em fence não conta
      if (decSeen.has(m[1]))
        err(relp, `DEC-id duplicado: ${m[1]} (também definido em ${decSeen.get(m[1])})`);
    ...
    if (tax && !tax.has(f.area))
      err(
        relp,
        `feature '${f.area}' não coberta pela TAXONOMY.md — R.13: adicione a linha '- ${f.area} — <definição>'`,
      );
  }
  ...
  for (const spec of listSpecs(root)) {
    ...
    if (!SPEC_RE.test(spec.id)) {
      err(relDir, "nome de pasta fora do padrão SPEC-YYYYMMDD-HHMM-slug (R.1)");
      continue;
    }
    const mainPath = P(spec.dir, "main.md");
    const mainRel = `${relDir}/main.md`;
    if (!exists(mainPath)) {
      err(relDir, "main.md ausente (arquivo canônico — R.3)");
      continue;
    }
    const pm = parseMain(read(mainPath));
    for (const k of ["Status", "Porte", "Owner", "Criada", "Keywords", "Features", "Resumo"]) {
      if (!(k in pm.fields) || pm.fields[k] === "")
        err(mainRel, `campo obrigatório ausente/vazio: **${k}:** (formato §3.1)`);
    }
    const st = pm.fields.Status;
    if (st && !STATUS_ENUM.includes(st))
      err(mainRel, `Status inválido '${st}' — enum: ${STATUS_ENUM.join(" | ")}`);
    if (pm.fields.Porte && !["P", "M", "G"].includes(pm.fields.Porte))
      err(mainRel, `Porte inválido '${pm.fields.Porte}' — use P | M | G`);
    ...
    if (st && STATUS_ENUM.includes(st) && !PHASE_STATUS[spec.phase].includes(st)) {
      err(
        mainRel,
        `coerência pasta↔status violada: ${spec.phase}/ exige status ${PHASE_STATUS[spec.phase].join(" ou ")}, encontrado '${st}'`,
      );
    }
    const tm = /^# SPEC-(\d{8}-\d{4})\b/.exec(pm.lines[0] || "");
    if (!tm) warn(mainRel, 'título fora do formato "# SPEC-YYYYMMDD-HHMM: <Título>"');
    else if (`SPEC-${tm[1]}` !== idPrefix(spec.id))
      warn(mainRel, `título (${tm[1]}) não bate com o id da pasta (${spec.id})`);
    for (const t of ["Objetivo", "Implementação"]) {
      if (!sectionRange(pm, (x) => x === t || x.startsWith(t + " ")))
        err(mainRel, `seção obrigatória ausente: ## ${t}`);
    }
    ...
    if (!esc) err(mainRel, "seção obrigatória ausente: ## Escopo (DENTRO/FORA)");
      ...
      if (!body.includes("DENTRO") || !body.includes("FORA"))
        warn(mainRel, "seção Escopo sem blocos DENTRO/FORA explícitos");
    }
    ...
    if (tax) {
      ...
      warn(
        mainRel,
        "docs/TAXONOMY.md ausente — vínculo **Features:** não validável (R.4); crie a TAXONOMY",
      );
    }
    if (!specFeats.length && st && st !== "draft")
      warn(mainRel, "**Features:** vazio — R.4 exige vínculo a feature antes da ativação");
    // checkboxes do main.md (qualquer [x] sem timestamp = ERRO — R.6)
    pm.lines.forEach((l) => {
      const m = /^\s*-\s*\[(x|X)\]\s*(.*)$/.exec(l);
      if (m && !hasTs(l)) {
        const msg = `checkbox [x] sem timestamp (R.6): "${truncate(m[2], 60)}"`;
        if (spec.v3) warn(mainRel, msg, { stage1: true });
        else err(mainRel, msg);
      }
    });
    const crit = getCriteria(pm);
    ...
    if (crit === null) err(mainRel, "seção obrigatória ausente: ## Critério de aceite");
      ...
      if (!crit.length)
        warn(mainRel, "Critério de aceite sem checkboxes — o contrato R.6.2 está vazio");
      ...
      if (porteLint === "P" && (spec.phase === "active" || spec.phase === "future") && !spec.v3) {
        for (const c of crit) {
          if (hasVerifyTag(c.raw))
            err(
              mainRel,
              `porte P não usa verify: — use specctl check (ou escalate para M): "${truncate(c.text, 60)}"`,
            );
        }
      }
      for (const c of crit) {
        if (c.checked) {
          const evid =
            (hasTs(c.raw) && hasCommitEv(c.raw)) || hasVerifyTag(c.raw) || hasEvidenceTag(c.raw);
          if (!evid) {
            const msg = `critério [x] sem evidência (timestamp+commit, verify: ou evidence:): "${truncate(c.text, 60)}" (R.6.2)`;
            if (spec.phase === "archive" && !spec.v3) err(mainRel, msg);
            else warn(mainRel, msg, { stage1: true });
          }
        } else if (spec.phase === "archive") {
          const critErr = spec.v3
            ? (mm) => warn(mainRel, mm, { stage1: true })
            : (mm) => err(mainRel, mm);
          if (!c.raw.includes("[aceito-incompleto"))
            critErr(
              `critério aberto em archive sem marcador [aceito-incompleto: "citação" YYYY-MM-DD HH:MM]: "${truncate(c.text, 60)}" (R.6.2 — só o USUÁRIO aceita incompleto)`,
            );
          else if (!ACEITO_RE.test(c.raw))
            critErr(
              `marcador [aceito-incompleto: ...] malformado — exige citação entre aspas + timestamp: "${truncate(c.text, 60)}"`,
            );
        }
      }
    }
    ...
    if (spec.phase === "archive" || spec.phase === "discard") {
      ...
      if (!exists(dg))
        err(
          `${relDir}/digest.md`,
          "digest.md obrigatório no fechamento (archive E discard) — ≤2.000 bytes",
        );
      else {
        ...
        if (b > 2000)
          err(
            `${relDir}/digest.md`,
            `orçamento excedido: ${b} > 2000 bytes (R.16) — digest é o resumo denso, não o journal`,
          );
      }
    }
    ...
    const needJournal =
      ...
      spec.phase === "archive" ||
      spec.phase === "discard" ||
      (spec.phase === "future" && st === "paused");
    if (needJournal && !exists(jp))
      err(
        `${relDir}/journal.md`,
        "journal.md ausente (arquivo canônico — R.3; pausada mantém main+journal)",
      );
    ...
    for (const key of POINTER_KEYS) {
      ...
      if (!exists(P(spec.dir, target)) && !exists(P(root, target))) {
        warn(mainRel, `ponteiro **${key}:** aponta para arquivo inexistente: ${target}`, {
          stage1: true,
        });
      }
    }
    ...
    if (exists(prdPath) && crit) {
      ...
      for (const n of [...rfs].sort((a, b) => a - b)) {
        if (!covered.has(n))
          err(
            mainRel,
            `RF-${n} do prd.md sem cobertura no Critério de aceite — use "(cobre RF-${n})" em ≥1 critério (rastreabilidade §3.2)`,
          );
      }
    }
  }
  ...
  for (const [f, max] of [
    ...
    if (exists(fp)) {
      ...
      if (b > max)
        E.push({
          f,
          m: `orçamento excedido: ${b} > ${max} bytes (R.16) — mova conteúdo para camada mais profunda (rollup, não corte)`,
        });
    }
  }
  ...
  // ROADMAP: entrada morta some da descoberta em silêncio — id com typo/arquivado/descartado é invisível no `next`
  {
    ...
    for (const id of parseRoadmap(root)) {
      if (!phaseOf.has(id)) {
        warn(
          "docs/ROADMAP.md",
          `cita ${id} que NÃO EXISTE (typo?) — a entrada é ignorada pelo specctl next`,
        );
        continue;
      }
      const ph = phaseOf.get(id),
        st = statusOf(id);
      if (ph === "archive" || ph === "discard")
        warn(
          "docs/ROADMAP.md",
          `cita ${id} já em ${ph}/ — remova a linha (entrada morta, ignorada pelo next)`,
        );
      else if (ph === "future" && st === "paused")
        warn(
          "docs/ROADMAP.md",
          `cita ${id} PAUSADA — retome via specctl resume ou remova do roadmap (paused não aparece no next)`,
        );
    }
  }
  // INDEX/ARCHIVE-INDEX/DEFERRED gerados e idempotentes (DEFERRED só warn quando ausente — repos v4.0/4.1 antigos não têm)
  for (const [f, gen] of [
    ["docs/INDEX.md", genIndex],
    ["docs/ARCHIVE-INDEX.md", genArchiveIndex],
    ["docs/DEFERRED.md", genDeferred],
  ]) {
    const fp = P(root, ...f.split("/"));
    if (!exists(fp)) {
      W.push({ f, m: "ausente — rode: node scripts/specctl.mjs index" });
      continue;
    }
    const txt = read(fp);
    if (!txt.includes(GERADO_MARK))
      E.push({ f, m: `marcador "${GERADO_MARK}" ausente — arquivo é GERADO, não edite à mão` });
    else if (norm(gen(root)) !== norm(txt))
      E.push({ f, m: "desatualizado (regeneração difere) — rode: node scripts/specctl.mjs index" });
  }

  // secret-scan em docs/ (R.15) — ERRO
  for (const fp of walkFiles(P(root, "docs"), {
    exts: [".md", ".txt", ".json", ".yml", ".yaml"],
  })) {
    for (const h of scanSecrets(readSafe(fp))) {
      E.push({
        f: relOf(root, fp),
        m: `segredo detectado (${h.tipo}) na linha ${h.line} — R.15: substitua por <REDACTED:${h.tipo}> e ROTACIONE a credencial`,
      });
    }
  }
  ...
  if (opts.targetMain) {
    ...
    if (act.length)
      E.push({
        f: "docs/active/",
        m: `R.2: docs/active/ deve estar vazio em main — encontrado: ${act.map((s) => s.id).join(", ")}`,
      });
  }
  return { E, W };
}
...
function cmdLint(flags) {
  ...
  if (flags.strict && !E.length && strictW.length)
    process.stdout.write(
      "(--strict: avisos contam como falha — exceto avisos de 1º release do §8)\n",
    );
...
function auditDeps(root, E, W) {
  ...
  for (const prog of parsePrograms(root)) {
    if (!prog.nodes.length) {
      W.push({ f: prog.rel, m: "programa sem nós (- SPEC-<id> | depende de: ...)" });
      continue;
    }
    if (/^\s*(\*\*)?Bloqueia/m.test(prog.txt))
      W.push({
        f: prog.rel,
        m: '"Bloqueia"/ordem são DERIVADOS do DAG — não escreva no manifesto (R.14)',
      });
    const known = new Map(prog.nodes.map((n) => [n.id, n]));
    for (const n of prog.nodes) {
      if (statusOf(n.id) === null)
        E.push({
          f: prog.rel,
          m: `nó inexistente no repositório: ${n.id} (R.14: todo nó do programa é uma SPEC real)`,
        });
      for (const d of n.deps) {
        if (statusOf(d) === null && !known.has(d))
          E.push({ f: prog.rel, m: `dependência inexistente: ${n.id} depende de ${d}` });
      }
    }
    // ciclos (DFS)
    const color = new Map();
    ...
    for (const n of prog.nodes) {
      if (!color.has(n.id)) visit(n.id);
      if (cycle) break;
    }
    if (cycle)
      E.push({
        f: prog.rel,
        m: `ciclo de dependências detectado: ${cycle.join(" → ")} (R.14: DAG obrigatório)`,
      });
    ...
    for (const n of prog.nodes) {
      ...
      for (const d of n.deps) {
        const ds = statusOf(d);
        if (st === "done" && ds && ds !== "done")
          W.push({
            f: prog.rel,
            m: `incoerência status×DAG: ${n.id} (done) depende de ${d} (${ds})`,
          });
        if (st === "active" && ds === "discarded")
          W.push({
            f: prog.rel,
            m: `incoerência status×DAG: ${n.id} (active) depende de ${d} (discarded)`,
          });
      }
    }
  }
}
const PIPELINE_ART_RE =
  /^docs\/(active|future|archive|discard)\/(SPEC-[^/]+)\/((?:\d+_task(?:_review)?|tasks|prd|techspec|bugs|qa-report|review-report|bugfix-report)\.md)$/;

function auditPr(root, base, E, W) {
  if (!base)
    die("audit --pr exige --base <sha> (base do PR) — sem base resolvível o gate falha FECHADO");
  ...
  for (const line of diff.split("\n").filter(Boolean)) {
    ...
    if (st.startsWith("R")) {
      ...
      if (st === "A") added.add(p);
      if (st === "D") {
        const sid = specIdFrom(p);
        if (sid) removedActive.add(sid);
      }
    }
  }
  const specs = listSpecs(root);
  for (const id of removedActive) {
    ...
    if (!loc) {
      E.push({
        f: `docs/active/${id}`,
        m: "SPEC saiu de docs/active/ e NÃO existe mais no repositório — R.5: nunca deletar; destinos válidos: archive/ (done), discard/ (com justificativa) ou future/ (pausada)",
      });
      continue;
    }
    ...
    if (loc.phase === "archive") {
      if (!exists(P(loc.dir, "digest.md")))
        E.push({ f: relMain, m: "archive sem digest.md — bloqueado (fechamento §3.4)" });
      const crit = getCriteria(pm) || [];
      for (const c of crit) {
        if (!c.checked && !ACEITO_RE.test(c.raw))
          E.push({
            f: relMain,
            m: `archive com critério aberto sem [aceito-incompleto: ...]: "${truncate(c.text, 60)}" (R.6.2)`,
          });
        if (
          c.checked &&
          !((hasTs(c.raw) && hasCommitEv(c.raw)) || hasVerifyTag(c.raw) || hasEvidenceTag(c.raw))
          ...
          E.push({
            f: relMain,
            m: `archive com critério [x] sem evidência: "${truncate(c.text, 60)}" (R.6.2)`,
          });
      }
      ...
      if (!pm.raw.includes("## Justificativa de descarte"))
        E.push({
          f: relMain,
          m: 'discard sem seção "## Justificativa de descarte" — bloqueado (R.5)',
        });
      if (!exists(P(loc.dir, "digest.md")))
        E.push({ f: relMain, m: "discard sem digest.md — bloqueado (lições preservadas, R.5)" });
    } else if (loc.phase === "future") {
      if ((pm.fields["Pausada em"] || "—").trim() === "—")
        E.push({
          f: relMain,
          m: "SPEC voltou a future/ sem **Pausada em:** preenchido — pausa legítima registra motivo (R.5)",
        });
    }
  }
  for (const p of added) {
    ...
    if (!m) continue;
    const sid = m[2];
    const journalTouched = [...touched].some(
      (t) => t.endsWith("/journal.md") && t.includes(`/${sid}/`),
    );
    if (!journalTouched)
      W.push({
        f: p,
        m: `artefato de pipeline sem contrapartida no journal da ${sid} (R.6.1) — registre [nota]/[decisão]/[descoberta] no LOG`,
      });
  }
  // arquivo no commit base (ausência = condição esperada de PR, não erro): retorna null sem AVISO
  const showAtBase = (relPath) => {
    const r = gitRun(["show", `${base}:${relPath}`], root);
  ...
  for (const s of specs.filter((x) => x.phase === "active")) {
    ...
    if (basePorte && headPorte && basePorte !== headPorte) {
      ...
      if (!/escalat|deescalat|porte/i.test(logPart)) {
        E.push({
          f: relMain,
          m: `campo Porte mudou (${basePorte} → ${headPorte}) sem entrada de journal (escalate/deescalate) — use node scripts/specctl.mjs escalate|deescalate (D2), nunca edite **Porte:** à mão`,
        });
      }
    }
  }
  ...
  if (specs.some((x) => x.phase === "active")) {
    const lg = gitRun(
      [
        "log",
        "--format=%h%x00%(trailers:key=Spec,valueonly,separator=%x2C)%x00%s",
        `${base}..HEAD`,
      ],
      root,
    );
    if (lg.ok) {
      ...
      if (missing.length)
        W.push({
          f: "(commits do PR)",
          m: `R.17: ${missing.length} commit(s) tocando código sob SPEC ativa SEM trailer 'Spec: SPEC-<id>': ${missing.slice(0, 3).join(" · ")}${missing.length > 3 ? ` (+${missing.length - 3})` : ""} — inclua o trailer nos próximos commits`,
        });
    }
  }
}
function auditBase(root, E, W) {
  ...
  for (const fp of walkFiles(P(root, "docs"), { exts: [".md"] })) {
    ...
    for (const m of txt.matchAll(SPEC_REF_RE)) {
      ...
      W.push({
        f: relOf(root, fp),
        m: `referência a SPEC fantasma: ${ref} (não existe em nenhuma fase)`,
      });
    }
  }
  ...
  for (const s of specs.filter((x) => x.phase === "active")) {
    ...
    if (!m) {
      W.push({
        f: `docs/active/${s.id}/journal.md`,
        m: "SNAPSHOT sem **Última atualização:** — staleness indeterminável",
      });
      continue;
    }
    const t = parseTs(m[1]);
    if (t && Date.now() - t.getTime() > 30 * DAY)
      W.push({
        f: `docs/active/${s.id}/journal.md`,
        m: `SPEC ativa parada há >30 dias (Última atualização: ${m[1]}) — pause com motivo ou retome`,
      });
  }
  ...
  for (const s of specs.filter((x) => x.phase === "active")) {
    ...
    if (porte === "P" && crit.length > 3) {
      W.push({
        f: `docs/active/${s.id}/main.md`,
        m: `porte P com ${crit.length} critérios — sinais de porte M; considere: node scripts/specctl.mjs escalate ${s.id} M`,
        stage1: true,
      });
    }
    ...
    if (crit.some((c) => /verify:\s*exit 0/.test(c.raw))) {
      ...
      if (!/verify:/.test(logPart)) {
        W.push({
          f: `docs/active/${s.id}/main.md`,
          m: `critério com "verify: exit 0" sem entrada de LOG contendo "verify:" no journal — evidência sem rastro (R.6.1); re-verifique com node scripts/specctl.mjs verify ${s.id} --all`,
          stage1: true,
        });
      }
    }
  ...
  if (isDir(claimsDir)) {
    for (const fp of walkFiles(claimsDir, { exts: [".md"] })) {
      ...
      if (elsewhere)
        W.push({
          f: relOf(root, fp),
          m: `claim órfão: ${cid} está em ${elsewhere.phase}/ — remova o claim (transições via specctl fazem isso)`,
        });
      // sem SPEC local: pode estar ativa em outra branch — é o design, não avisa
    }
    for (const id of activeIds) {
      if (!exists(claimPath(root, id)))
        W.push({
          f: `docs/active/${id}`,
          m:
            "ativação sem claim (R.11) — recrie: docs/claims/" +
            id +
            ".md (specctl activate faz isso)",
        });
    }
    ...
    if (manifest(root).team === "multi") {
      for (const id of activeIds) {
        ...
        const cTxt = read(cp);
        const base =
          ((/^\*\*Base:\*\*\s*(.+)$/m.exec(cTxt) || [])[1] || "").trim() ||
          detectBaseBranch(root, currentBranch(root));
        if (!base || base === "—") continue;
        const rel = relOf(root, cp);
        const inBase = gitRun(["cat-file", "-e", `${base}:${rel.split(path.sep).join("/")}`], root);
        const inRemoteBase = inBase.ok
          ? inBase
          : gitRun(["cat-file", "-e", `origin/${base}:${rel.split(path.sep).join("/")}`], root);
        if (!inBase.ok && !inRemoteBase.ok) {
          W.push({
            f: rel,
            m: `team=multi: claim de ${id} ainda NÃO publicado em '${base}' — os outros devs não veem o trabalho em voo (R.11). PR trivial: só docs/claims/ (specctl activate imprime os comandos)`,
          });
        }
      }
    }
  ...
  for (const f of featureFiles(root)) {
    for (const line of norm(readSafe(f.path)).split("\n")) {
      ...
      if (Date.now() - t.getTime() > 180 * DAY)
        W.push({
          f: relOf(root, f.path),
          m: `decisão ativa há >180 dias: ${m[1]} — revalide ou marque obsoleta`,
        });
    }
  }
}
function cmdAudit(flags) {
  ...
  if (flags["main-gate"]) {
    ...
    if (act.length)
      E.push({
        f: "docs/active/",
        m: `R.2 (gate de main): docs/active/ deve estar vazio — encontrado: ${act.map((s) => s.id).join(", ")}`,
      });
  }
...
function briefNextLines(root, disc, activeNonEmpty) {
  ...
  if (activeNonEmpty) {
    return [
      `Backlog: ${openPrograms.length} programa(s) aberto(s), ${roadmapCount} no roadmap — \`specctl next\``,
    ];
  }
  ...
  if (disc.orphans.length) {
    L.push(
      `- Não-priorizadas: ${disc.orphans.length} (rode \`specctl next\` ou priorize em docs/ROADMAP.md)`,
    );
  }
...
function cmdBrief(flags) {
  ...
  if (!root) {
    out("[SDD v4] projeto sem docs/.spec-system.json — rode a skill spec-system-init.");
    process.exit(0);
  }
  ...
  if (flags.agent) {
    ...
    const B = [];
    B.push(
      `[bundle ${spec.id} — 1-call p/ subagente · branch ${branch || "(git indisponível)"} · NOW=${now()}]`,
    );
    B.push("");
    ...
    try {
      B.push(checklistText(spec, closeStage(root, spec), true));
    } catch {
      B.push(`fechar: node scripts/specctl.mjs close ${spec.id} --dry`);
    }
    out(B.join("\n"));
    process.exit(0);
  }
  ...
  if (flags.compact) {
    ...
    L.push("");
    L.push(
      `Branch: ${branch || "(git indisponível)"} · SPECs ativas: ${act.length ? act.map((s) => s.id).join(", ") : "nenhuma"}`,
    );
    if (act.length) {
      L.push("");
    ...
    L.push("");
    L.push(
      "(pré-compact: faça o flush — sobrescreva o SNAPSHOT e registre no LOG o que não pode se perder)",
    );
    L.push("");
  ...
  L.push("");
  L.push(
    "Referências: criar/editar artefato → docs/rules/formats.md · transição de ciclo → docs/rules/lifecycle.md · programa/DAG → docs/rules/programs.md · interop/workspace → docs/rules/interop.md",
  );
  // INDEX filtrado pelas Features da SPEC ativa (+ "outras: ver docs/INDEX.md")
  const idxAll = norm(readSafe(P(root, "docs", "INDEX.md")))
    .split("\n")
  ...
  if (!idxAll.length) {
    ...
    const mine = idxAll.filter((l) => {
      const m = /^-\s+\*\*([^*]+)\*\*/.exec(l);
      return m && myFeats.has(m[1].trim());
    });
  ...
  if (!act.length) L.push("SPECs ativas: nenhuma (docs/active/ vazio)");
    ...
    for (const s of act) {
      ...
      L.push(
        `SPEC ativa: ${s.id} (${info.feats.join(", ") || "sem features"}) — critérios ${info.done}/${info.total} [x]`,
      );
    }
    if (act.length > 1)
      L.push(
        "ATENÇÃO: múltiplas SPECs em docs/active/ — indique QUAL no prompt (R.9 exige escolha).",
      );
    L.push("");
    L.push(`== SNAPSHOT (${act[0].id}) ==`);
    L.push(snapOf(act[0]));
  ...
  if (isDir(claimsDir)) {
    for (const fp of walkFiles(claimsDir, { exts: [".md"] })) {
      ...
      if (cf.some((x) => myFeats.has(x)))
        collisions.push(`- ${cid} (${owner}) — features: ${cf.join(", ")}`);
    }
  }
  L.push("");
  L.push(
    collisions.length
      ? "Claims em COLISÃO (mesma feature em voo em outra branch):"
      : "Claims em colisão: nenhuma",
  );
  ...
  if (defCount)
    L.push(`Deferidos (R.6.2, com citação): ${defCount} item(ns) — ver docs/DEFERRED.md`);
...
function cmdNext(root, flags) {
  ...
  if (flags.program) {
    ...
    if (!programs.some((p) => p.prog.slug === slug)) {
      die(
        `programa '${slug}' não existe em docs/programs/ — programas: ${programs.map((p) => p.prog.slug).join(", ") || "(nenhum)"}`,
      );
    }
  ...
  for (const { prog, state } of programs) {
    ...
    for (const b of state.blocked)
      progLines.push(
        `  🔒 bloqueado: ${b.id} (programa ${prog.slug} — aguarda ${b.waits.join(", ")})`,
      );
    for (const id of state.paused)
      progLines.push(`  ⏸ pausado:  ${id} (programa ${prog.slug} — retomar: specctl resume ${id})`);
  }
  if (progLines.length) {
    L.push("");
  ...
  if (!flags.program && model.orphans.length) {
    L.push("");
    L.push(
      "BACKLOG NÃO-PRIORIZADO (future sem programa e fora do ROADMAP — defina ordem em docs/ROADMAP.md):",
    );
  ...
  const hasContent =
    progLines.length ||
    (!flags.program && (model.roadmap.next || model.roadmap.later.length || model.orphans.length));
  if (!hasContent) {
    if (flags.program)
...
function cmdCapsule() {
  ...
  const inlineNote =
    man.interop === "inline" && act.length === 1
...
function cmdVerify(pos, flags) {
  ...
  if (!targets.length) {
    out(
      "nada a verificar — nenhum critério pendente com `verify:` executável (use --all para reverificar)",
    );
    process.exit(0);
  }
  ...
  for (const c of targets) {
    ...
    try {
      ...
      detail = truncate(String(e.stderr || e.stdout || e.message || ""), 300);
    }
    if (ok) {
      let line = lines[c.line];
      line = line.replace(
        /\s*\(\d{4}-\d{2}-\d{2} \d{2}:\d{2}, commit `[^`]*`, verify: exit 0\)/,
        "",
      );
      const vm = /^(\s*-\s*)\[(?: |x|X)\]\s*(.*?)(\s*\|\s*verify:.*)$/.exec(line);
      if (vm)
        lines[c.line] =
          `${vm[1]}[x] ${vm[2]} (${now()}, commit \`${short}\`, verify: exit 0)${vm[3]}`;
      process.stdout.write("  PASS — marcado [x] com evidência\n");
      summary.push(`PASS: ${truncate(c.text, 60)}`);
    } else {
      fails++;
      process.stdout.write(
        `  FAIL — critério permanece [ ]\n${detail ? "  " + detail + "\n" : ""}`,
      );
  ...
  appendLog(
    spec.dir,
    "nota",
    `verify: ${targets.length - fails}/${targets.length} critérios passaram (commit \`${short}\`)`,
...
function sanitizeBody(buf, label) {
  ...
  try {
    ...
    die(`${label}: conteúdo não é UTF-8 válido — reenvie como UTF-8 (sem BOM)`);
  }
  return s
    .replace(/^\uFEFF/, "")
    .replace(/\r\n?/g, "\n")
...
function cmdLog(pos, flags) {
  const root = requireRoot();
  if (!pos[0] || !pos[1] || pos[2] === undefined)
    die(`log exige <id> <tipo> "título".\n${LOG_HELP}`);
  const spec = findSpec(root, pos[0]);
  const tipo = pos[1];
  if (!LOG_TYPES.includes(tipo))
    die(
      `tipo inválido '${tipo}' — gramática completa dos tipos: ${LOG_TYPES.join(" | ")}\n${LOG_HELP}`,
    );
  const titulo = String(pos[2]);
  if (/[\r\n]/.test(titulo))
    die(
      `título é single-line na gramática do LOG — detalhe multilinha vai no corpo (--stdin/--body-file).\n${LOG_HELP}`,
    );
...
function cmdCheck(pos, flags) {
  ...
  const spec = findSpec(root, pos[0]);
  if (spec.phase !== "active")
    die(`check: ${spec.id} está em ${spec.phase}/ — evidência de critério só em SPEC ativa`);
  const n = parseInt(pos[1], 10);
  const mainPath = P(spec.dir, "main.md");
  const pm = parseMain(read(mainPath));
  const crit = getCriteria(pm);
  if (!crit || !crit.length)
    die(`${spec.id}: seção "## Critério de aceite" sem checkboxes — nada a estampar`);
  if (!Number.isInteger(n) || n < 1 || n > crit.length) {
    die(
      `uso: node scripts/specctl.mjs check <id> <n> [--evidence "..."] — n é 1-based; ${spec.id} tem ${crit.length} critério(s)`,
    );
  }
  const c = crit[n - 1];
  if (hasVerifyTag(c.raw))
    die(`critério ${n} tem verify: — rota: node scripts/specctl.mjs verify ${spec.id}`);
  const ev = flags.evidence ? String(flags.evidence).trim() : "";
  if (/[\r\n]/.test(ev))
    die("--evidence é single-line — detalhe longo vai no LOG (specctl log) ou em evidence/");
  if (/verify:/.test(ev))
    die(
      'check NUNCA escreve o token "verify:" — evidência executável usa: node scripts/specctl.mjs verify',
    );
  const lines = norm(read(mainPath)).split("\n");
  const allClosedMsg = () => {
  ...
  if (!m)
    die(`critério ${n}: linha fora do formato "- [ ] ..." — corrija o main.md antes de estampar`);
  lines[c.line] =
    `${m[1]}[x] ${m[2]} (${now()}, commit \`${short}\`${ev ? `, evidence: ${ev}` : ""})`;
  writeFile(mainPath, lines.join("\n"));
  out(lines[c.line].trim());
  allClosedMsg();
}
...
function genDigest(root, spec, o = {}) {
  ...
  for (const m of j.matchAll(
    /^## \d{4}-\d{2}-\d{2} \d{2}:\d{2} — (?:\[MARCO\] )?\[conclusão\]\s*(.+)$/gm,
  ))
    conclusao = m[1].trim();
  if (SNAP_PLACEHOLDER.test(onde) || onde === "—")
    onde = conclusao || (SNAP_PLACEHOLDER.test(onde) ? "(ver critérios/decisões)" : onde);
  const dec = [];
  for (const m of j.matchAll(
    /^## \d{4}-\d{2}-\d{2} \d{2}:\d{2} — (?:\[MARCO\] )?\[decisão\]\s*(.+)$/gm,
  ))
    dec.push(truncate(m[1], 100));
  ...
  const build = () => {
    if (porte === "P") {
      // porte P: 3-5 linhas, sem passo do modelo
    ...
    const L = [
      `# Digest — ${spec.id}${titulo ? `: ${truncate(titulo, 80)}` : ""}`,
      DIGEST_STUB_NOTE,
      "",
    ];
    L.push(`**Resumo:** ${resumo}`);
    L.push(
      `**Porte:** ${porte} · **Features:** ${f.Features || "—"} · **Criada:** ${f.Criada || "—"} · **Concluída:** ${concluida}`,
    );
    L.push(`**Critérios:** ${done}/${crit.length} fechados · **Commit final:** ${commitFinal}`);
    L.push(`**Entregue:** ${entregue}`);
    if (decs.length) {
      L.push("**Decisões:**");
      for (const d of decs) L.push(`- ${d}`);
    }
    L.push("");
  ...
  let txt = build();
  while (digestMeasure(txt) > 2000) {
    // nunca devolve o corte pro modelo: decisões → entregue → resumo
    if (decs.length) decs.pop();
    else if (entregue.length > 60)
      entregue = truncate(entregue, Math.max(60, Math.floor(entregue.length / 2)));
    else if (resumo.length > 60)
      resumo = truncate(resumo, Math.max(60, Math.floor(resumo.length / 2)));
    else break;
    txt = build();
  }
...
function cmdDigest(pos, flags) {
  ...
  if (flags.stdin || flags.file) {
    // refino do modelo: re-validado com resposta acionável
    ...
    if (b > 2000)
      die(
        `digest recusado: ${b}B > 2000B: corte ~${b - 2000}B e reenvie (medição LF, comentários HTML descontados)`,
      );
  ...
  if (exists(dgPath) && !flags.fix) {
    ...
    if (b > 2000)
      die(
        `digest.md existe com ${b}B > 2000B — re-gere dentro do cap: node scripts/specctl.mjs digest ${spec.id} --fix`,
      );
    die(
      `digest.md já existe (${b}B, dentro do cap) — refine via --stdin/--file ou re-gere: node scripts/specctl.mjs digest ${spec.id} --fix`,
    );
  }
...
function featurePathOf(root, area) {
  return P(root, "docs", "features", ...area.split("/")) + ".md";
}
...
function cmdRollup(pos) {
  ...
  if (!area)
    die(
      "uso: node scripts/specctl.mjs rollup <area> — move blocos antigos p/ docs/features/<area>.history.md até ≤8.000B",
    );
  const r = rollupArea(root, area);
  out(
    `rollup: ${r.moved} bloco(s) → docs/features/${area}.history.md${r.moved ? "" : " (nada movido)"}`,
  );
  out(
    r.fit
...
function cmdEscalate(pos) {
  ...
  const spec = findSpec(root, pos[0]);
  if (spec.phase === "archive" || spec.phase === "discard")
    die(`escalate: ${spec.id} está em ${spec.phase}/ — Porte só muda em SPEC aberta`);
  const alvo = String(pos[1] || "").toUpperCase();
  if (!["M", "G"].includes(alvo))
    die("uso: node scripts/specctl.mjs escalate <id> <M|G> (one-way: P→M→G)");
  const mainPath = P(spec.dir, "main.md");
  ...
  if (PORTE_RANK[alvo] <= PORTE_RANK[cur]) {
    die(
      `escalate é ONE-WAY (P→M→G): ${cur} → ${alvo} não é escalação. Rebaixar é decisão do USUÁRIO: node scripts/specctl.mjs deescalate ${spec.id} <P|M> --cita "citação literal do usuário"`,
    );
  }
...
function cmdDeescalate(pos, flags) {
  ...
  const spec = findSpec(root, pos[0]);
  if (spec.phase === "archive" || spec.phase === "discard")
    die(`deescalate: ${spec.id} está em ${spec.phase}/ — Porte só muda em SPEC aberta`);
  const alvo = String(pos[1] || "").toUpperCase();
  if (!["P", "M"].includes(alvo))
    die('uso: node scripts/specctl.mjs deescalate <id> <P|M> --cita "citação literal do usuário"');
  const cita = flags.cita ? String(flags.cita).trim() : "";
  if (!cita)
    die(
      'deescalate exige --cita "citação literal do usuário" — rebaixar Porte é decisão do USUÁRIO (padrão aceito-incompleto), nunca do agente',
    );
  const mainPath = P(spec.dir, "main.md");
  const cur = (parseMain(read(mainPath)).fields.Porte || "M").toUpperCase();
  if (PORTE_RANK[alvo] >= PORTE_RANK[cur])
    die(
      `deescalate rebaixa o Porte: ${cur} → ${alvo} não é rebaixamento — para subir use: node scripts/specctl.mjs escalate ${spec.id} ${alvo}`,
    );
  writeFile(mainPath, setField(read(mainPath), "Porte", alvo));
  const body = [`[aceito-incompleto: "${cita.replace(/"/g, "'")}" ${now()}]`, gitFooter(root)]
    .filter(Boolean)
    .join("\n");
...
function closeStage(root, spec) {
  ...
  let verifyPending = 0;
  if (!crit.length)
    blockers.push('seção "## Critério de aceite" sem checkboxes — contrato R.6.2 vazio');
  crit.forEach((c, i) => {
    const n = i + 1;
    if (hasVerifyTag(c.raw)) {
      const m = /\|\s*verify:\s*`([^`]+)`/.exec(c.raw);
      ...
      if (!c.checked && !ACEITO_RE.test(c.raw)) {
        if (runnable) {
          verifyPending++;
          autos.push(`critério ${n} com verify: pendente — será executado no close`);
        } else
          blockers.push(
            `critério ${n} com verify: placeholder — preencha o comando real (ou fluxo R.6.2): "${truncate(c.text, 50)}"`,
          );
      }
    } else if (!c.checked && !ACEITO_RE.test(c.raw)) {
      blockers.push(
        `critério ${n} aberto sem evidência: "${truncate(c.text, 50)}" — rota: node scripts/specctl.mjs check ${spec.id} ${n} ou fluxo R.6.2 (item a item, só o USUÁRIO aceita incompleto)`,
      );
    } else if (c.checked && !hasTs(c.raw)) {
      blockers.push(
        `critério ${n} [x] sem timestamp (R.6): "${truncate(c.text, 50)}" — evidencie via check/verify`,
      );
    }
  });
  const jp = P(spec.dir, "journal.md");
  ...
  if (!jtxt || !/^## \d{4}-\d{2}-\d{2} \d{2}:\d{2} — (?:\[MARCO\] )?\[conclusão\]/m.test(jtxt)) {
    blockers.push(
      `[conclusão] ausente no LOG — rota: node scripts/specctl.mjs log ${spec.id} conclusão "o que foi entregue"`,
    );
  }
  ...
  if (SNAP_PLACEHOLDER.test(snapEnd >= 0 ? jtxt.slice(0, snapEnd) : jtxt)) {
    blockers.push(
      `SNAPSHOT ainda com placeholder do template ("Onde tô/Próximo passo/Fase") — sobrescreva o SNAPSHOT refletindo a conclusão antes de fechar (o digest é gerado dele)`,
    );
  }
  const feats = splitList(pm.fields.Features || "");
  ...
  if (closeTestCmd)
    autos.push(
      `suíte do projeto roda 1× no close (${truncate(closeTestCmd, 40)}) — falha aborta sem mover nada`,
    );
...
function cmdClose(pos, flags) {
  ...
  const spec = findSpec(root, pos[0]);
  if (spec.phase === "archive") {
    out(`close: ${spec.id} já está em docs/archive/ — nada a fazer (idempotente)`);
    process.exit(0);
  }
  if (spec.phase === "discard")
    die(`close: ${spec.id} está em discard/ — SPEC descartada não fecha`);
  if (spec.phase === "future") {
    const st0 = parseMain(readSafe(P(spec.dir, "main.md"))).fields.Status || "draft";
    die(
      `close exige SPEC em active/ — ${spec.id} está em future/ (status ${st0}). Rota: node scripts/specctl.mjs ${st0 === "paused" ? "resume" : "activate"} ${spec.id}, termine o trabalho e rode o close.`,
    );
  }
  const st = closeStage(root, spec);
  if (flags.dry) {
    out(checklistText(spec, st, true));
    process.exit(st.blockers.length ? 1 : 0);
  }
  if (st.blockers.length) {
    out(checklistText(spec, st, false));
    process.exit(1);
  }
  ...
  if (vTotal) {
    ...
    if (r.code !== 0) {
      ...
      die(
        `close ABORTADO: verify falhou (relato acima). Critérios aprovados mantêm a evidência estampada — corrija e rode o close de novo.`,
      );
    }
    vPass = vTotal;
  }
  ...
  if (testCmd) {
    ...
    try {
      execSync(testCmd, { cwd: root, stdio: "pipe", timeout: 600000 });
      testNote = "PASS";
    } catch (e) {
      const detail = truncate(String(e.stdout || "") + String(e.stderr || e.message || ""), 1500);
      process.stderr.write(detail.endsWith("\n") ? detail : detail + "\n");
      die(
        `close ABORTADO: suíte do projeto falhou (${testCmd}). Corrija e rode o close de novo — nada foi movido.`,
      );
    }
  }
  ...
  writeFile(
    P(spec.dir, "main.md"),
    setField(read(P(spec.dir, "main.md")), "Commit final", `\`${short}\``),
  );
  ...
  if (exists(cp0)) {
    const mb = /^\*\*Base:\*\*\s*(.+)$/m.exec(read(cp0));
    if (mb) baseBranch = mb[1].trim();
  }
  ...
  out(
    `verify ${vPass}/${vTotal} · suite ${testNote} · digest ${dgBytes}B · lint ${E.length}/${W.length} · audit ${AE.length}/${AW.length}`,
  );
  ...
  out(
    `— finalização (o modelo PERGUNTA ao usuário e executa; nada automático) — branch ${curBranch} · base ${baseBranch}`,
  );
  out(
    `  1. commitar; depois: (a) merge direto em ${baseBranch}  OU  (b) push + abrir PR para ${baseBranch}`,
  );
...
function cmdAdoptWorkspace(pos) {
  ...
  if (ws === "inline") {
    out("workspace inline — artefatos já vivem na pasta da SPEC, nada a adotar");
    process.exit(0);
  }
  if (!ws || ws === "—")
    die(
      `${spec.id} sem **Workspace:** declarado no main.md — nada a adotar (modo external registra ex.: tasks/prd-<slug>/)`,
    );
  ...
  appendLog(
    spec.dir,
    "nota",
    `adopt-workspace: snapshot de ${ws} copiado para a pasta da SPEC (${count} arquivo(s)) — workspace original preservado`,
  );
  out(
    `adotado: ${count} arquivo(s) de ${ws} → ${relOf(root, spec.dir)} (snapshot pré-archive; o workspace NÃO foi movido)`,
  );
}
...
function cmdEntrypoints() {
  ...
  for (const kind of ["CLAUDE.md", "AGENTS.md"]) {
    ...
    out(
      `${kind}: ${b} bytes${b > max ? ` — AVISO: acima do orçamento de ${max} (R.16); reduza o bloco projeto` : ""}`,
    );
  }
}
...
function cmdGuardRead() {
  ...
  if (tool === "Grep" || tool === "Glob") {
    ...
    if (
      (rel === "docs/archive" || rel.startsWith("docs/archive/")) &&
      !allowlisted(root, act, targetId)
    ) {
      deny(
        `[guard-read] BLOQUEADO (R.8 via ${tool}): busca dentro de docs/archive/ — N3 é acessado via digest/ARCHIVE-INDEX ou subagente (retorno ≤500 tokens).`,
      );
    }
    if (
      targetId &&
      ...
      deny(
        `[guard-read] BLOQUEADO (R.8 via ${tool}): busca dentro de docs/${tm[1]}/${targetId} — SPEC fora do escopo ativo (main.md/digest.md podem ser LIDOS direto; o resto via subagente ou allowlist).`,
      );
    }
    process.exit(0);
  }
  ...
  if (/(^|\/)scripts\/specctl\.mjs$/.test(rel) && revGte(man.template_revision, "4.1.0")) {
    deny(
      `[guard-read] BLOQUEADO (v4.1): não leia scripts/specctl.mjs para "garantir compliance" — os gates dizem o que falta.\nUse: node scripts/specctl.mjs --help  ·  node scripts/specctl.mjs close <id> --dry`,
    );
  }
  if (rel.startsWith("..") || !rel.startsWith("docs/")) process.exit(0);
  ...
  if (rel.startsWith("docs/archive/")) {
    ...
    deny(
      exitMsg("conteúdo de docs/archive/ — N3 é acessado via digest/ARCHIVE-INDEX ou subagente"),
    );
  }
  if (
    ["journal.md", "state.md", "memory.md"].includes(base) &&
    targetId &&
    !activeIds.has(targetId)
  ) {
    if (allowlisted(root, act, targetId)) process.exit(0);
    deny(
      exitMsg(
        `${base} pertence a SPEC fora do escopo ativo (ativa: ${act.map((s) => s.id).join(", ") || "nenhuma"})`,
      ),
    );
  }
      ...
      out(
        JSON.stringify({
          hookSpecificOutput: {
            ...
            permissionDecisionReason: `policy=strict: leitura N1 de ${rel} exige confirmação do usuário (R.10 estrito). main.md/digest.md são sempre livres.`,
          },
        }),
      );
      process.exit(0);
...
function isTempName(base) {
  return TEMP_EXTS.includes(path.extname(base).toLowerCase()) || TEMP_PREFIX.test(base);
}
...
const CTX_WIN_STD = 200000,
  CTX_WIN_1M = 1000000;
const CTX_TH_STD = 160000,
  CTX_TH_1M = 250000,
  CTX_STEP = 60000;
function ctxLatestTokens(transcriptPath) {
  ...
  try {
    ...
    for (let i = lines.length - 1; i >= (start > 0 ? 1 : 0); i--) {
      // linha 0 de tail truncado = JSON parcial
      ...
      if (t > 0)
        return {
          tokens: t,
          model: rec.message && typeof rec.message.model === "string" ? rec.message.model : "",
        };
    }
    return null;
  } catch {
...
function maybeSuggestCompact(input, root) {
  try {
    ...
    const win =
      Number.isInteger(envWin) && envWin > 0
        ? envWin
        : Number.isInteger(manWin) && manWin > 0
          ? manWin
          : usage.model.includes("[1m]") || usage.tokens > CTX_WIN_STD
            ? CTX_WIN_1M
            : CTX_WIN_STD;
    ...
    const th =
      Number.isInteger(parsedTh) && parsedTh > 0
        ? parsedTh
        : win >= CTX_WIN_1M
          ? CTX_TH_1M
          : CTX_TH_STD;
    ...
    let last = -1;
    try {
      const p = parseInt(fs.readFileSync(stateFile, "utf8").trim(), 10);
      if (Number.isInteger(p) && p >= 0) last = p;
    } catch {
      /* primeiro disparo */
    }
    if (bucket <= last) return null;
    try {
      fs.writeFileSync(stateFile, String(bucket));
    } catch {
      /* sem estado, sem re-lembrete — aceitável */
    }
    try {
      // sweep de estado >14d de outras sessões — nunca falha o hook
      ...
      for (const e of fs.readdirSync(os.tmpdir())) {
        ...
        try {
          if (fs.statSync(sf).mtimeMs < cut) fs.rmSync(sf, { force: true });
        } catch {
          /* ignora */
        }
      }
...
function cmdGuardWrite() {
  ...
  const base = path.basename(rel);
  if (
    ["docs/INDEX.md", "docs/ARCHIVE-INDEX.md", "docs/PROGRAMS.md", "docs/DEFERRED.md"].includes(rel)
  ) {
    deny(
      `[guard-write] BLOQUEADO: ${rel} é GERADO. Caminho correto: edite a fonte (features/TAXONOMY/programs/critérios) e rode: node scripts/specctl.mjs index`,
    );
  }
  if (isTempName(base) && !inAllowedTempDir(rel)) {
    ...
    deny(
      `[guard-write] BLOQUEADO (R.12): '${base}' parece temporário/artefato fora de área permitida. Caminho correto: ${hint}/tmp/ (descartável) ou ${hint}/evidence/ (persistente); sem SPEC ativa: .scratch/.`,
    );
  }
  if (rel.startsWith("docs/active/")) {
    ...
    if (isProtectedBranch(branch, root)) {
      deny(
        `[guard-write] BLOQUEADO (R.2): escrita em ${rel} na branch protegida '${branch}'. Caminho correto: crie uma branch (git checkout -b feature/<slug>) e escreva lá; rascunho sem branch vai em docs/future/.`,
      );
    }
  }
  ...
  const editTexts = () => {
    ...
    if (tool === "MultiEdit" && Array.isArray(ti.edits))
      return {
        newText: ti.edits.map((e) => e.new_string || "").join("\n"),
  ...
  if (gated && rel.startsWith("docs/")) {
    ...
    for (const h of scanSecrets(String(newText))) {
      ...
      deny(
        `[guard-write] BLOQUEADO (R.15 — pré-write): SEGREDO (${h.tipo}) no conteúdo destinado a ${rel}. Docs NUNCA guardam segredos — escreva <REDACTED:${h.tipo}> no lugar; se a credencial já vazou em outra via, ROTACIONE.`,
      );
    }
  }
  ...
  if (gated && base === "journal.md") {
    ...
    if (heads.length) {
      deny(
        `[guard-write] BLOQUEADO (v4.1 — Escriba Único): entrada nova no ## LOG só via specctl log (a ferramenta carimba NOW+commit+diffstat). Trecho: '${truncate(heads[0], 60)}'.\n` +
          `Caminho correto:\n` +
  ...
  if (gated && base === "main.md" && rel.startsWith("docs/active/")) {
    ...
    for (const m of nt.matchAll(spoofRe)) {
      ...
      deny(
        `[guard-write] BLOQUEADO (v4.1 anti-spoofing): só specctl verify grava evidência "verify: exit 0" em critério — a ferramenta roda o comando e carimba na hora. Trecho: '${truncate(line, 60)}'.\n` +
  ...
  if (base === "journal.md" && tool !== "Write") {
    const olds =
      tool === "MultiEdit" && Array.isArray(ti.edits)
        ? ti.edits.map((e) => e.old_string || "")
        : [ti.old_string || ""];
    ...
    if (txt) {
      ...
      if (logIdx >= 0) {
        ...
        while ((m = headRe.exec(txt)) !== null) {
          if (m.index > logIdx) lastIdx = m.index;
        }
        if (lastIdx >= 0) {
          for (const oldS of olds) {
            ...
            if (posOld > logIdx && posOld < lastIdx) {
              deny(
                `[guard-write] BLOQUEADO (TIER-0): o LOG do journal é append-only — proibido editar entradas antigas (trecho '${truncate(oldS, 60)}' está antes da última entrada do LOG). Caminho correto: adicione uma NOVA entrada '## ${now()} — [tipo] ...' ao FINAL do arquivo (o SNAPSHOT e a última entrada podem ser editados).`,
              );
            }
          }
        }
  ...
  if (gated) {
    ...
    if (sug)
      out(
        JSON.stringify({
          hookSpecificOutput: { hookEventName: "PreToolUse", additionalContext: sug },
        }),
      );
  }
  process.exit(0);
...
function cmdStampCheck() {
  ...
  // Write sob docs/: valida o content COMPLETO (timestamps inventados no início também contam — R.6)
    ...
    newText = underDocs ? ti.content : ti.content.split(/\r?\n/).slice(-40).join("\n");
  ...
  const oldText =
    typeof ti.old_string === "string"
      ...
      : Array.isArray(ti.edits)
        ? ti.edits.map((e) => e.old_string || "").join("\n")
        : "";
  const drift = [];
  ...
  if (drift.length)
    out(
      `[stamp-check] AVISO (R.6): timestamp(s) fora de ±24h do relógio (NOW=${now()}): ${drift.slice(0, 3).join(", ")} — use {{NOW}} ou o NOW injetado, nunca invente data.`,
    );
  ...
  if (hits.length) {
    const h = hits[0];
    deny(
      `[stamp-check] SEGREDO detectado (${h.tipo}, linha ${h.line}) em ${rel}. R.15: docs NUNCA guardam segredos — substitua AGORA por <REDACTED:${h.tipo}> e ROTACIONE a credencial exposta (exceção auditada ao append-only cobre a remediação).`,
    );
  }
  process.exit(0);
}
...
function cmdSessionClose() {
  ...
  if (st.error || st.status !== 0) {
    process.stderr.write(
      `[session-close] AVISO: git status falhou: ${(st.stderr || (st.error && st.error.message) || "").trim()} — verificação de journal pulada\n`,
    );
    process.exit(0);
  }
...
function stBase(tmp, name) {
  ...
  if (!g.ok)
    process.stderr.write(
      `[self-test] AVISO: git init falhou (${g.err}) — fixtures seguem sem git\n`,
    );
  return root;
}
...
function stJournal(idTs, { conclusao = false, snapshotDone = false } = {}) {
  ...
  if (snapshotDone)
    j = j
      .replace("início — nada feito ainda", "feature entregue")
      .replace("<primeiro passo concreto>", "—")
      .replace("<fase>", "impl");
...
function stArchiveSpec(root, { digest = "ok", criterios, journal } = {}) {
  ...
  writeFile(
    ...
    stMain(ST_ARCH, {
      status: "done",
      concluida: "2026-01-12 12:00",
      commitFinal: "`abc1234`",
      criterios: criterios || [
        "- [x] Funciona de ponta a ponta (2026-01-12 11:50, commit `abc1234`, verify: exit 0)",
      ],
    }),
  );
...
function stManifest40(root) {
  writeFile(
    P(root, "docs", ".spec-system.json"),
    JSON.stringify(
      {
        schema: SCHEMA,
        template_revision: "4.0.0",
        installed_at: "2026-01-01 10:00",
        policy: "standard",
        interop: "none",
        product: null,
...
function cmdSelfTest() {
  ...
  const expectRun = (r, code, substr, label) => {
    expect(
      r.code === code,
      `${label}: exit ${r.code} (esperado ${code}) — saída: ${truncate(r.out, 400)}`,
    );
    if (substr)
      expect(
        r.out.includes(substr),
        `${label}: saída sem "${substr}" — saída: ${truncate(r.out, 400)}`,
      );
  };
  ...
  fixture("future-malformada", () => {
    ...
    writeFile(
      P(root, "docs", "future", id, "main.md"),
      stMain("20260501-1200", { status: "active" }),
    );
  ...
  fixture("discard-legitimo", () => {
    ...
    writeFile(
      P(dir, "main.md"),
      stMain("20260301-1500", {
        status: "discarded",
        criterios: ["- [ ] não implementado"],
        extra:
  ...
  fixture("segredo-plantado", () => {
    ...
    writeFile(
      P(root, "docs", "features", "core.md"),
      stFeatureCore("", "chave usada: AKIAABCDEFGHIJKLMNOP\n"),
    );
  ...
  fixture("deps-ciclicas", () => {
    ...
    writeFile(
      P(root, "docs", "future", a, "main.md"),
      stMain("20260401-1000", { status: "draft", ativada: "—" }),
    );
    writeFile(
      P(root, "docs", "future", b, "main.md"),
      stMain("20260401-1100", { status: "draft", ativada: "—" }),
    );
    writeFile(
      P(root, "docs", "programs", "alpha.md"),
      `# Programa: alpha\n\n**Owner:** @tester\n\n- ${a} | depende de: ${b}\n- ${b} | depende de: ${a}\n`,
    );
  ...
  fixture("guard-read-nega", () => {
    ...
    writeFile(
      P(root, "docs", "future", bid, "main.md"),
      stMain("20260201-0800", { status: "paused", pausada: "2026-02-02 10:00 — aguardando API" }),
    );
    writeFile(P(root, "docs", "future", bid, "journal.md"), stJournal("20260201-0800"));
    const payload = JSON.stringify({
      tool_name: "Read",
      tool_input: { file_path: P(root, "docs", "future", bid, "journal.md") },
    });
  ...
  fixture("guard-write-index-gerado", () => {
    ...
    const payload = JSON.stringify({
      tool_name: "Write",
      tool_input: { file_path: P(root, "docs", "INDEX.md"), content: "x" },
    });
  ...
  fixture("guard-write-temporario", () => {
    ...
    const payload = JSON.stringify({
      tool_name: "Write",
      tool_input: { file_path: P(root, "saida.log"), content: "log" },
    });
  ...
  fixture("guard-write-suggest-compact", () => {
    ...
    const payload = JSON.stringify({
      tool_name: "Write",
      tool_input: { file_path: P(root, "docs", "nota.md"), content: "oi" },
      transcript_path: tp,
      session_id: sid,
    });
    ...
    expect(
      !r2.out.includes("additionalContext"),
      "guard-write: re-sugeriu no MESMO bucket (estado por sessão falhou)",
    );
    ...
    expect(
      !r3.out.includes("additionalContext"),
      "guard-write: ignorou context_window do manifesto (sugeriu a 176k numa janela 1M)",
    );
  });
  fixture("stamp-check-now", () => {
    ...
    const payload = JSON.stringify({
      tool_name: "Write",
      tool_input: { file_path: fp, content: "Atualizado: {{NOW}}\n" },
    });
  ...
  fixture("next-programa", () => {
    ...
    writeFile(
      P(root, "docs", "archive", raiz, "main.md"),
      stMain("20260401-0900", {
        status: "done",
        concluida: "2026-04-02 10:00",
        commitFinal: "`abc1234`",
      }),
    );
    writeFile(
      P(root, "docs", "archive", raiz, "journal.md"),
      stJournal("20260401-0900", { conclusao: true }),
    );
    writeFile(P(root, "docs", "archive", raiz, "digest.md"), "# Digest\n\nRaiz concluída.\n");
    writeFile(
      P(root, "docs", "future", pronto, "main.md"),
      stMain("20260401-1000", { status: "draft", ativada: "—" }),
    );
    writeFile(
      P(root, "docs", "future", blk, "main.md"),
      stMain("20260401-1100", { status: "draft", ativada: "—" }),
    );
    writeFile(
      P(root, "docs", "programs", "demo.md"),
    ...
    expect(
      r.out.includes("🔒") && r.out.includes(blk),
      `next: nó bloqueado ausente — saída: ${truncate(r.out, 400)}`,
    );
    expect(
      r.out.includes(`aguarda ${pronto}`),
      `next: motivo do bloqueio ausente — saída: ${truncate(r.out, 400)}`,
    );
  });
  fixture("next-roadmap", () => {
    ...
    writeFile(
      P(root, "docs", "future", r1, "main.md"),
      stMain("20260501-1000", { status: "draft", ativada: "—" }),
    );
    writeFile(
      P(root, "docs", "future", r2, "main.md"),
      stMain("20260502-1000", { status: "draft", ativada: "—" }),
    );
    writeFile(P(root, "docs", "ROADMAP.md"), `# ROADMAP\n\n1. ${r1}\n2. ${r2}\n`);
    const r = runCli(root, ["next"]);
    expectRun(r, 0, "ROADMAP", "next");
    expect(
      r.out.includes(`▶ próximo:  ${r1}`),
      `next: topo do roadmap ausente — saída: ${truncate(r.out, 400)}`,
    );
    expect(
      r.out.includes("· depois:") && r.out.includes(r2),
      `next: "depois" do roadmap ausente — saída: ${truncate(r.out, 400)}`,
    );
  });
  fixture("programs-index", () => {
    ...
    writeFile(
      P(root, "docs", "archive", feito, "main.md"),
      stMain("20260601-0800", { status: "done" }),
    );
    writeFile(
      P(root, "docs", "future", raiz, "main.md"),
      stMain("20260601-0900", { status: "draft", ativada: "—" }),
    );
    writeFile(
      P(root, "docs", "future", filho, "main.md"),
      stMain("20260601-1000", { status: "draft", ativada: "—" }),
    );
    writeFile(
      P(root, "docs", "programs", "demo.md"),
    ...
    expect(
      first.includes("## demo — aberto · 1/3 · owner @tester"),
      `programs-index: cabeçalho do bloco incorreto — saída: ${truncate(first, 400)}`,
    );
    expect(
      first.includes(`- prontos: ${raiz}`),
      `programs-index: linha "prontos" ausente — saída: ${truncate(first, 400)}`,
    );
    expect(
      first.includes(`- bloqueados: ${filho} (aguarda ${raiz})`),
      `programs-index: linha "bloqueados" ausente — saída: ${truncate(first, 400)}`,
    );
    expect(
      first.includes(`- concluídos: ${feito}`),
      `programs-index: linha "concluídos" ausente — saída: ${truncate(first, 400)}`,
    );
    runCli(root, ["index"]); // regenera
    expect(
      read(p) === first,
      "programs-index: regeneração NÃO é idempotente (diff no PROGRAMS.md)",
    );
    // nó PAUSADO aparece na linha própria (visível, não pegável) — e no next com ⏸
    const pausado = "SPEC-20260601-1100-idx-pausado";
    writeFile(
      P(root, "docs", "future", pausado, "main.md"),
      stMain("20260601-1100", { status: "paused", pausada: "2026-06-02 09:00 — aguardando API" }),
    );
    ...
    const second = read(p);
    expect(
      second.includes(`- pausados: ${pausado} (retomar: specctl resume)`),
      `programs-index: linha "pausados" ausente — saída: ${truncate(second, 400)}`,
    );
    const rn = runCli(root, ["next"]);
    expect(
      rn.out.includes("⏸ pausado") && rn.out.includes(pausado),
      `next: nó pausado ausente — saída: ${truncate(rn.out, 400)}`,
    );
  });
  fixture("log-scribe", () => {
    ...
    expect(
      bad.out.includes(LOG_TYPES.join(" | ")),
      `gramática completa ausente no erro — saída: ${truncate(bad.out, 300)}`,
    );
    expect(
      bad.out.includes("PowerShell") && bad.out.includes("EOF"),
      "erro sem exemplos heredoc/here-string",
    );
    const multi = runCli(root, ["log", id, "nota", "linha1\nlinha2"]);
    expectRun(multi, 1, "single-line", "log título multilinha");
    const r = runCli(
      root,
      ["log", id, "decisão", "Optamos por X", "--stdin"],
      "porque Y\r\nlinha dois\n",
    );
    ...
    expect(
      r.out.trim().split("\n").length <= 2,
      `eco acima de 2 linhas — saída: ${truncate(r.out, 300)}`,
    );
    const j = norm(read(P(root, "docs", "active", id, "journal.md")));
    expect(
      /— \[decisão\] Optamos por X\n\nporque Y\nlinha dois\n⎿ commit [0-9a-f]+/.test(j),
      `entrada/rodapé fora do formato — fim do journal: ${truncate(j.slice(-260), 260)}`,
    );
  ...
  fixture("check-estampa", () => {
    ...
    const line = norm(read(P(root, "docs", "active", id, "main.md")))
      .split("\n")
      .find((l) => l.includes("validado manualmente"));
    expect(
      /^- \[x\] validado manualmente com o usuário \(\d{4}-\d{2}-\d{2} \d{2}:\d{2}, commit `[0-9a-f]+`, evidence: aprovado pelo usuário\)$/.test(
        line,
      ),
      `estampa incorreta: ${line}`,
    );
  ...
  fixture("digest-gera-e-cap", () => {
    ...
    expect(
      digestMeasure(read(dg)) <= 2000,
      `digest acima de 2000B (LF, HTML descontado): ${digestMeasure(read(dg))}`,
    );
    ...
    expect(
      /\d+B > 2000B/.test(big.out),
      `mensagem acionável ausente — saída: ${truncate(big.out, 300)}`,
    );
  ...
  fixture("rollup-orcamento", () => {
    ...
    const filler = "linha de contexto acumulado que ocupa espaço no arquivo da feature. ".repeat(
      11,
    );
    let deltas = "";
    for (let i = 1; i <= 12; i++)
      deltas += `- SPEC-202601${String(i).padStart(2, "0")}-1000 — delta ${i}: ${filler}\n`;
    writeFile(
      P(root, "docs", "features", "core.md"),
      `# Feature: core
...
`,
    ...
    expect(
      bytesOf(read(P(root, "docs", "features", "core.md"))) > 8000,
      "fixture deve começar acima de 8000B",
    );
    ...
    expect(
      after.includes("decisão viva que NÃO pode sair"),
      'rollup tocou "Decisões arquiteturais ativas"',
    );
  ...
  fixture("escalate-one-way", () => {
    ...
    expect(
      norm(read(P(root, "docs", "active", id, "main.md"))).includes("**Porte:** G"),
      "Porte não atualizado no main.md",
    );
    expect(
      norm(read(P(root, "docs", "active", id, "journal.md"))).includes(
        "[nota] Porte escalado P → G",
      ),
      "[nota] de escalação ausente no LOG",
    );
    ...
    expect(
      norm(read(P(root, "docs", "active", id, "main.md"))).includes("**Porte:** M"),
      "Porte não rebaixado",
    );
    const j = norm(read(P(root, "docs", "active", id, "journal.md")));
    expect(
      /\[aceito-incompleto: "pode simplificar, M basta" \d{4}-\d{2}-\d{2} \d{2}:\d{2}\]/.test(j),
      `citação no padrão aceito-incompleto ausente — fim do journal: ${truncate(j.slice(-260), 260)}`,
    );
  });
  fixture("close-pendencias", () => {
    ...
    expect(
      dry.out.includes(`check ${id} 1`),
      `rota do check ausente — saída: ${truncate(dry.out, 400)}`,
    );
  ...
  fixture("close-completo", () => {
    ...
    const id = stActiveSpec(root, {
      criterios: ['- [ ] roda ponta a ponta | verify: `node -e "process.exit(0)"`'],
    });
    writeFile(
      P(root, "docs", "active", id, "journal.md"),
      stJournal(ST_ACT, { conclusao: true, snapshotDone: true }),
    );
    ...
    expect(
      r.out.includes("verify 1/1"),
      `atestado sem "verify 1/1" — saída: ${truncate(r.out, 400)}`,
    );
    expect(
      r.out.includes("nada mais a rodar"),
      `atestado sem rodapé — saída: ${truncate(r.out, 400)}`,
    );
    expect(
      r.out.includes("finalização") && r.out.includes("deletar a branch"),
      `atestado sem menu de finalização git — saída: ${truncate(r.out, 500)}`,
    );
    ...
    expect(
      exists(P(root, "docs", "active", ".gitkeep")),
      "docs/active/ sem .gitkeep após close — sumiria em checkout fresco (CI)",
    );
    const main = norm(read(P(root, "docs", "archive", id, "main.md")));
    expect(
      main.includes("**Status:** done") && /\*\*Commit final:\*\* `[0-9a-f]+`/.test(main),
      "Status/Commit final não estampados",
    );
  ...
  fixture("close-snapshot-stale", () => {
    ...
    const id = stActiveSpec(root, {
      criterios: ["- [x] feito (2026-01-12 12:00, commit `abc1234`)"],
    });
    ...
    writeFile(
      jp,
      read(jp)
        .replace("início — nada feito ainda", "entregue")
        .replace("<primeiro passo concreto>", "—")
        .replace("<fase>", "impl"),
    );
  ...
  fixture("protected-branch", () => {
    ...
    expect(
      /\*\*Base:\*\*\s*\S/.test(claim) && !/\*\*Base:\*\*\s*—/.test(claim),
      `claim sem Base detectada — ${truncate(claim, 200)}`,
    );
  });
  ...
  fixture("lint-porte-p-verify", () => {
    ...
    stActiveSpec(root, {
      porte: "P",
      criterios: ['- [ ] roda ponta a ponta | verify: `node -e "0"`'],
    });
    writeFile(P(root, "docs", "features", "core.md"), stFeatureCore(""));
  ...
  fixture("makeMain-porte-p-scaffold", () => {
    ...
    expect(
      !/\|\s*verify:/.test(main.slice(main.indexOf("## Critério"))),
      "scaffold P não deve ter slot verify:",
    );
    expect(
      main.includes("<!-- P: feche com: node scripts/specctl.mjs check"),
      "comentário auto-doc P ausente",
    );
  ...
  fixture("guard-read-specctl", () => {
    ...
    const payload = JSON.stringify({
      tool_name: "Read",
      tool_input: { file_path: P(root, "scripts", "specctl.mjs") },
    });
    ...
    stManifest40(root);
    expectRun(
      runCli(root, ["guard-read"], payload),
      0,
      null,
      "guard-read permite specctl em 4.0.0",
    );
  });
  fixture("guard-write-log-append", () => {
    ...
    const payload = JSON.stringify({
      tool_name: "Edit",
      tool_input: { file_path: jp, old_string: anchor, new_string: newEntry },
    });
    const d = runCli(root, ["guard-write"], payload);
    expectRun(d, 2, "specctl log", "guard-write nega LOG-append (4.1.0)");
    expect(
      d.out.includes("--stdin") && d.out.includes("--body-file"),
      "deny sem copy-paste --stdin/--body-file",
    );
    // SNAPSHOT/prosa continuam livres (Edit sem heading de LOG)
    const okSnap = JSON.stringify({
      tool_name: "Edit",
      tool_input: {
        file_path: jp,
        old_string: "**Onde tô:** início — nada feito ainda",
        new_string: "**Onde tô:** avancei bastante",
      },
    });
    expectRun(
      runCli(root, ["guard-write"], okSnap),
      0,
      null,
      "guard-write permite edição de SNAPSHOT",
    );
    // com revision 4.0.0: permite o append (repo v4.0 não quebra)
    stManifest40(root);
    expectRun(
      runCli(root, ["guard-write"], payload),
      0,
      null,
      "guard-write permite LOG-append em 4.0.0",
    );
  });
  fixture("guard-write-verify-spoof", () => {
    ...
    const spoof =
      '- [x] critério pendente (2026-01-12 10:00, commit `abc1234`, verify: exit 0) | verify: `node -e "process.exit(0)"`';
    const payload = JSON.stringify({
      tool_name: "Edit",
      tool_input: { file_path: mp, old_string: oldC, new_string: spoof },
    });
    const d = runCli(root, ["guard-write"], payload);
    ...
    expectRun(
      ...
      0,
      null,
      "guard-write permite verify-spoof em 4.0.0",
    );
  });
  fixture("brief-agent", () => {
    ...
    expect(
      r.out.includes("== main.md ==") && r.out.includes("== SNAPSHOT"),
      "bundle sem main.md/SNAPSHOT",
    );
  ...
  fixture("audit-anti-fuga", () => {
    ...
    writeFile(P(root, "docs", "features", "core.md"), stFeatureCore(""));
    stActiveSpecId(root, "20260701-1000", "porte-p-grande", {
      porte: "P",
      criterios: ["- [ ] c1", "- [ ] c2", "- [ ] c3", "- [ ] c4"],
    });
    stActiveSpecId(root, "20260701-1100", "evidencia-spoof", {
      porte: "M",
      criterios: [
        '- [x] roda (2026-01-12 10:00, commit `abc1234`, verify: exit 0) | verify: `node -e "0"`',
      ],
    });
    const r = runCli(root, ["audit"]);
    expectRun(r, 0, "sinais de porte M", "audit anti-fuga: P com >3 critérios");
    expect(
      r.out.includes("evidência sem rastro"),
      `audit "evidência sem rastro" ausente — saída: ${truncate(r.out, 500)}`,
    );
  });
  fixture("audit-pr-porte-diff", () => {
    ...
    writeFile(
      P(root, "docs", "active", id, "main.md"),
      stMain("20260701-1200", { status: "active", porte: "M", criterios: ["- [ ] c1"] }),
    );
    ...
    writeFile(
      P(root, "docs", "active", id, "main.md"),
      stMain("20260701-1200", { status: "active", porte: "G", criterios: ["- [ ] c1"] }),
    );
    writeFile(
      P(root, "docs", "features", "core.md"),
      stFeatureCore("") + "\n" + "x".repeat(8500) + "\n",
    ...
    expect(
      !r.out.includes("ratchet"),
      `audit --pr NÃO deve mais ratchetar tamanho de feature — saída: ${truncate(r.out, 500)}`,
    );
  });
  ...
  fixture("roadmap-entrada-morta", () => {
    ...
    const r = runCli(root, ["lint"]);
    expect(
      r.out.includes("NÃO EXISTE"),
      `lint sem warn de id inexistente no ROADMAP — saída: ${truncate(r.out, 400)}`,
    );
    expect(
      r.out.includes("já em archive/"),
      `lint sem warn de entrada morta (arquivada) no ROADMAP — saída: ${truncate(r.out, 400)}`,
    );
  });
  fixture("guard-write-segredo-pre", () => {
    ...
    const payload = JSON.stringify({
      tool_name: "Edit",
      tool_input: {
        file_path: P(root, "docs", "features", "core.md"),
        old_string: "x",
        new_string: "chave: AKIAABCDEFGHIJKLMNOP",
      },
    });
    expectRun(
      runCli(root, ["guard-write"], payload),
      2,
      "R.15",
      "guard-write nega segredo PRÉ-write",
    );
  });
  ...
  fixture("team-multi-claim", () => {
    ...
    expect(
      ra.out.includes(`claim/${id}`) && ra.out.includes("PR trivial"),
      `coreografia ausente — saída: ${truncate(ra.out, 400)}`,
    );
    // capsule carrega team=multi
    const rc = runCli(root, ["capsule"]);
    expect(
      rc.out.includes("team=multi"),
      `capsule sem team=multi — saída: ${truncate(rc.out, 200)}`,
    );
    ...
    expect(
      rau.out.includes("NÃO publicado"),
      `audit sem aviso de claim não-publicado — saída: ${truncate(rau.out, 400)}`,
    );
  ...
  fixture("fence-nao-esconde-criterio", () => {
    ...
    expectRun(
      ...
      1,
      "aceito-incompleto",
      "lint DEVE ver o critério aberto escondido atrás do fence",
    );
    expect(
      !r.out.includes("EXEMPLO dentro do fence") || !/aceito-incompleto[^\n]*EXEMPLO/.test(r.out),
      "checkbox de exemplo dentro do fence não deveria ser tratado como critério",
    );
  });
  fixture("status-mesmo-minuto", () => {
    ...
    writeFile(
      P(root, "docs", "archive", a, "main.md"),
      stMain("20260101-1000", {
        status: "done",
        concluida: "2026-01-02 12:00",
        commitFinal: "`abc1234`",
        criterios: ["- [x] ok (2026-01-02 11:50, commit `abc1234`, verify: exit 0)"],
      }),
    );
    writeFile(
      P(root, "docs", "archive", a, "journal.md"),
      stJournal("20260101-1000", { conclusao: true }),
    );
    writeFile(P(root, "docs", "archive", a, "digest.md"), "# Digest\n\nok.\n");
    writeFile(
      P(root, "docs", "future", b, "main.md"),
      stMain("20260101-1000", { status: "draft", ativada: "—" }),
    );
    writeFile(
      P(root, "docs", "programs", "gamma.md"),
      `# Programa: gamma\n\n**Owner:** @tester\n\n- ${a} | depende de: —\n- ${b} | depende de: ${a}\n`,
    );
    ...
    expect(
      r.out.includes(b) && /▶|pronto/i.test(r.out),
      `nó b deveria estar PRONTO (dep done) — saída: ${truncate(r.out, 300)}`,
    );
  });
  fixture("fence-nao-prova-r7", () => {
    ...
    const id = stActiveSpec(root, {
      porte: "P",
      criterios: ["- [x] Feito (2026-01-10 11:00, commit `abc1234`) — check"],
    });
...
`,
    ...
    expect(
      r.code === 1 && r.out.includes("R.7"),
      `close --dry DEVE bloquear R.7 com id só em fence — saída: ${truncate(r.out, 300)}`,
    );
  });
  fixture("close-roda-suite", () => {
    ...
    expect(
      rf.code === 1 && rf.out.includes("suíte do projeto falhou"),
      `close deveria abortar com suíte falhando — saída: ${truncate(rf.out, 300)}`,
    );
    expect(
      isDir(P(root, "docs", "active", id)),
      "SPEC não pode ter sido movida com suíte falhando",
    );
  ...
  if (allOk) {
    try {
      ...
      out(`(fixtures mantidas em ${tmp} — remoção falhou)`);
    }
...
try {
  switch (cmd) {
    ...
    case "session-close":
      cmdSessionClose();
      break;
    case "adopt-workspace":
      cmdAdoptWorkspace(pos);
      break;
    case "entrypoints":
      cmdEntrypoints();
      break;
  ...
  die(
    `falha inesperada em '${cmd}': ${e.message}${process.env.SPECCTL_DEBUG ? "\n" + e.stack : ""}`,
  );
}
